...
/**
 * Directories that hold no repo content. `node_modules`, `.git` and `dist` are
 * build and dependency output; `.claude` and `.superpowers` are gitignored
 * working directories - agent worktrees and SDD task reports - that exist only
 * on a machine that has run those workflows. A clean checkout has neither, so
 * omitting them was invisible until this suite ran on a machine that did.
 *
 * This is a coverage sweep over what the repo SHIPS, and an ignored scratch
 * directory was never in that set: an exemption here weakens nothing, whereas
 * walking into one makes the sweep fail for reasons no reader can act on.
 */
const NOT_REPO_CONTENT = ['node_modules', '.git', 'dist', '.claude', '.superpowers']
...
async function walkMarkdown(dir: string, prefix = ''): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (NOT_REPO_CONTENT.includes(entry.name)) continue
